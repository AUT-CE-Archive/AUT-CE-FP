...
static void robberRandomWalk(Cop[]);
static void copsWalk(Cop[]);
static bool canIGoTo(int, int, bool, Cop[]);
...
static int robbersMoves = 0;
static int copsMoves = 0;
...
void action(Cop cops[]) {
	...
	while (!gameOver(cops)) {
		resetConsole();
		robberRandomWalk(cops);		// Move Robber Randomly
		robbersMoves++;
		if (gameOver(cops))
			isCaught = true;
	...
	SetColor(7);
	printf("Total Number Of Generations: %d\nTotal Robber Moves: %d\nTotal Cops Moves: %d\n", generations, robbersMoves, copsMoves);
}
...
void copsWalk(Cop cops[]) {
	...
	for (int i = 0; i < TotalCopsCount; i++) {
		...
		if (cops[i].X - robber.X != 0) {
			...
			if (canIGoTo(cops[i].X + velocityX, cops[i].Y + velocityY, false, cops) == true) {
				cops[i].X += velocityX;
				cops[i].Y += velocityY;
			} else
				goto z;
...
void robberRandomWalk(Cop cops[]) {
	...
	if (canIGoTo(robber.X + velocityX, robber.Y + velocityY, true, cops) == true) {
		robber.X += velocityX;
		robber.Y += velocityY;
	} else
		goto z;
...
bool canIGoTo(int X, int Y, bool isRobber, Cop cops[]) {
...
void drawMatrix(Cop cops[]) {
	...
	matrix[robber.X][robber.Y] = (!isCaught) ? robber.symbol : 'X';

	// Draw Matrix
	for (int i = 0; i < rowCount; i++) {
		for (int j = 0; j < columnCount; j++) {

			if (matrix[i][j] == 'T' && !isCaught)
				SetColor(robber.colorID);	// Robbers Collor
			else if (matrix[i][j] == 'T' && isCaught)
				SetColor(12);
